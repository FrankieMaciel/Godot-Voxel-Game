...
public partial class Player : CharacterBody3D
{
            ...
            if (@event.IsPressed()) {
                switch (m.ButtonIndex) {
                    ...
                    case MouseButton.Right:
                        if (playerRay.IsColliding()) {
                            Config.world_node.updateChunkBlockInfo(playerRay.GetCollisionPoint(), playerRay.GetCollisionNormal(), 1);
                        }
                    break;
    ...
    public override void _PhysicsProcess(double delta)
    {
        ...
        if (direction != Vector3.Zero) {
            ...
            n_velocity.X = Mathf.MoveToward(n_velocity.X, 0, SPEED);
            n_velocity.Z = Mathf.MoveToward(n_velocity.Z, 0, SPEED);
        }
        Velocity = n_velocity;
        MoveAndSlide();
    }
